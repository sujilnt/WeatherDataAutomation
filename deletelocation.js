...
const weatherFunc = (dataObj, count)=>{
	...
	puppeteer.launch(_launchSetting).then(async browser => {
		...
		let siteVame= await _siteName.toString();
		await page.setViewport(_viewport);
		await page.evaluate((siteVame) => {
			console.log(siteVame);
		},siteVame);
		...
		await page.type("input.search-query.search",_siteName + String.fromCharCode(13));
		...
		await page.$$eval("#right td", searchText=>{
			let d3= Array.from(searchText).filter((r)=>{
				return r.innerHTML.includes("PFS");
			});
			d3[0].click();
		});
